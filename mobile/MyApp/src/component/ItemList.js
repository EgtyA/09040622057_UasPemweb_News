...
const ItemList = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get('http://192.168.1.10:5000/items'); // Ganti dengan IP yang benar
  ...
  const renderItem = ({ item }) => (
    <View style={styles.itemContainer}>
      ...
      <Text style={styles.itemText}>Kategori: {item.nama_kategori}</Text>
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent: 'center', // Mengatur konten di tengah vertikal
